...
function constructState(pointsMarkdown) {
  return produce(initialState, (draft) => {
    ...
    //   draft.completionStreak = [draft.completionStreak.at(-1)];
    // }
    ...
    draft.taskList.weekly = weeklyTasks
      .split("\n")
      .filter((s) =>
        s.startsWith(
          "#on-" +
...
function applyCompletionStreak(state) {
  ...
  //   parseCompletionGoal(state.completionStreak.at(-1)) ?? {};
  // return produce(state, (draft) => {
...
const newState = applyTabs(
  sortActiveTasks(
    applyStreaks(
      applyCompletedTasks(
        console.tap(constructState(tasks.replace(/  /g, " ")))
      )
    ),
...
//   const [dateTag] = Object.entries(dates).find(([dateTag, taskStrs]) => {
//     return getTime(...taskStrs) + taskTime <= TODAY_HOUR_LIMIT;
//   }) ?? ["none"];
...
newState.activeTasks = {
  weekly: newState.taskList.weekly,
  active: newState.taskList.active.reduce((acc, p) => {
    acc[p.quadrant] = (acc[p.quadrant] ?? []).concat(p.str);
    return acc;
  }, {})
};
