...
import { parsedToString } from "../parsers/alias";
...
| | |
| ---------- | ----------- | -------------- |
${Object.values(state.alias).map(parsedToString).join("\n")}
...
${state.taskList.done
  ...
  .map((t) => t.replace("- [x] ", ""))
  .join("\n")}
${state.activeTasks.weekly.join("\n")}

${Object.entries(state.activeTasks.active)
  .flatMap(([q, tasks]) => {
    return [`### ${q}`, `${tasks.map((t) => t).join("\n")}`, ""];
  })
  .join("\n")}
